import React from 'react'

function MatContextualizando() {
  return (
    <div className='max-w-7xl mx-auto p-8 m-4'>
        <div className="p-4 mb-4">
            <h1 className="text-4xl font-semibold text-slate-700 mb-4">Contextualizando o ensino de Matemática</h1>
            <p className="text-slate-800">
            Quando iniciamos novos conteúdos, são inúmeras as vezes em que os alunos perguntam <strong>mas onde vou usar isso em minha vida?</strong> ou <strong>para quê serve isso?</strong>. Estas perguntas surgem quando os alunos não fazem conexões entre o conteúdo e sua realidade e/ou necessidade em usá-lo. Para conseguir superar essa situação é necessário que o ensino de Matemática seja contextualizado, isto é, que faça sentido para a realidade daquele indivíduo. <a href="https://drive.google.com/file/d/1YGuqNR3ZsgFrm8vmEzyC7yvixnC08e0h/view?usp=sharing" target="_blank" rel="noopener noreferrer" className="underline text-blue-800 hover:text-purple-800"> Veja aqui</a>{' '}como a professora Vilma e o professor André contextualizaram suas aulas de Matemática.
            </p>
        </div>

        <div className="max-w-5xl mx-auto items-center justify-center py-8">
              <div className="attention-box mx-8 text-slate-800 text-center text-lg border-2 border-primary rounded-lg p-8">
              <h1 className="text-4xl font-semibold text-slate-700 mb-4">Atividades Extras</h1>
                <p>
                    Acesse <a href="https://conteudoextra.vercel.app/" target="_blank" rel="noopener noreferrer" className="underline text-blue-800 hover:text-purple-800"> nosso site de atividades extras</a>{' '}com atividades que vão além do conteúdo presente nos livros físicos. Nele, você poderá acessar e baixar PDFs relacionados ao conteúdo de cada capítulo, dando um passo além no ensino de habilidades matemáticas.
                </p>
                
              </div>
        </div>

    </div>
  )
}

export default MatContextualizando